...
import { setGeneralInfo, GeneralInfo, setFormcontractInfo, ContractInfo, setSaleInfo, SaleInfo, setParticipationCriteria, ParticipationCriteria } from '../../../store';
...
function InputForm() {
    ...
    const { formData, userAddress, networkID } = useSelector((state: any) => state)
    ...
    const [times, setTimes] = useState<{startingTime: number, endingTime: number}>({
        startingTime: Math.floor(Date.now()) + 1*60*60*1000 , 
    ...
    const handleTokenomics = (e: any, type: string) => {
        if(type === "tokensForSale"){
            setTokenomics({ tokensForSale: e, tokensForLiquidity: tokenomincs.tokensForLiquidity })
        } else if(type === "tokensForLiquidity"){
            setTokenomics({ tokensForSale: tokenomincs.tokensForSale, tokensForLiquidity: e })
        }
    }
    // useEffect(() => {
    ...
    //             "https://api.bscscan.com/api?module=contract&action=getabi&address=0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82&apikey=NP3ZXMF51W5G48FMV9T5GGC1K67YV5UEXC"
    //             )
                
    //             var Contract = await new web3.eth.Contract(JSON.parse(res.data.result), "0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82");
    ...
    //             // console.log( `symbol ${symbol} , totalSupply ${totalSupply}, name ${name}, decimals ${decimals}, balanceOf ${balanceOf}` )
    ...
    return (
        <div>
            <div style={{ padding: "5px" }}>
                                                ...
                                                formData?.contractInfo?.name ?
                                                    ...
                                                    <>
                                                    {
                                                        ...
                                                        <div>
                                                            ...
                                                            0x5e9e3400fdae0a64ed3eeeb4cc5f97c002dd5618 <br />
                                                        </div>
                                                        :
                                                        null
                                                    }
                ...
                <Formik
                    initialValues={{ criteriaAddress: '', type: 0, tokensForLiquidity: 0,  tokensForSale: 0}}
                    // validationSchema={schema1} 
                    onSubmit={async (values, { setFieldValue }) => {
                ...
                <Formik
                    initialValues={{ priceOfToken: 0.01, minTokensForParticipation: 250, minReqTokens: 50, maxReqTokens: 100, softCap: tokenomincs.tokensForSale/2,  }}
                    // validationSchema={schema1} 
                                        ...
                                        <Grid container spacing={1}>
                                            ...
                                            <Grid item xs={6} sx={{ margin: 0 }} >
                                                <Field
                                                    ...
                                                    type="number"
                                                    name="softCap"
                                                    label={`Soft cap for success of the project (should be more than ${tokenomincs.tokensForSale/2})`}
                                                    fullWidth
                                                />
                                            </Grid>
                                        {
                        ...
                        <>
                            ...
                            <Grid item xs={12}
                                ...
                                <Button
                                    ...
                                    size="small"
                                    sx={{ margin: 2 }}
                                    disabled = {tokenApproval ? true: false}

                                >
                                    <div >Approve {formData?.saleInfo.tokensForSale * ( 1 + formData?.saleInfo?.tokensForLiquidity / 100 ) } Tokens</div>
                                ...
                                <Button
                                    ...
                                    className="form-button"
                                    sx={{ margin: 2 }}
                                    disabled = {tokenApproval ? false: true}
                                >
