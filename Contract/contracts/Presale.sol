...
contract Presale is Ownable{
    ...
    modifier onlyPresaleOwner(uint _id) {
        require(presaleContractStatic[_id].ownerOfPresale == _msgSender(), "Ownable: caller is not the owner of this presale");
        _;
    }
    ...
    function buyTokensOnPresale(uint256 _id, uint256 _numOfTokensRequested) payable public isIDValid(_id) isPresaleActive(_id){
        ...
        address _criteriaToken =  presaleContractStatic[_id].criteriaTokenAddr;

        if(_criteriaToken != address(0)){
            uint _critriaTokenUserHold = IERC20(_criteriaToken).balanceOf(msg.sender);
            require(_critriaTokenUserHold >= currentProject.minTokensForParticipation, "Not enough tokens to participate. Should be atleast 250");
        }
